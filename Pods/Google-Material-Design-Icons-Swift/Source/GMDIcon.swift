...
private class FontLoader {
    ...
    static let loadFontIfNeeded: Void = {
        if (UIFont.fontNames(forFamilyName: GMDStruct.FontName).count == 0) {
            let bundle = Bundle(for: FontLoader.self)
            let cocoapodsURL = bundle.url(
                forResource: GMDStruct.FileFontName,
                withExtension: "ttf",
                subdirectory: "Google-Material-Design-Icons-Swift.bundle"
            )
            let normalURL = bundle.url(forResource: GMDStruct.FileFontName, withExtension: "ttf")

            guard let identifier = bundle.bundleIdentifier,
                let url = identifier.hasPrefix("org.cocoapods") ? cocoapodsURL : normalURL,
